...
from .m2m_setup import M2MSetupWizard
from .utils import (
    console,
    get_az_account_info,
...
class SetupWizard:
    ...
    def run(self) -> bool:
        ...
        # Step 6: Password configuration
        password_strategy, vault_name = self._configure_password_strategy(
            default_region, resource_group_prefix
        )

        # Step 7: M2M Bearer Token Authentication (Optional)
        m2m_wizard = M2MSetupWizard()
        m2m_config = m2m_wizard.run()
...
You can run this setup again anytime with: [cyan]uv run bicep-deploy setup[/cyan]
    ...
    def _configure_resource_naming(self) -> str:
        ...
        console.print("Example: [cyan]neo4j-test-standalone-v2025-20260120-143052[/cyan]")
    ...
    def _show_summary(self, settings: Settings) -> None:
        ...
        if settings.m2m and settings.m2m.enabled:
            table.add_row("M2M Authentication", "Enabled")
            table.add_row("M2M Audience", settings.m2m.audience or "Not set")
